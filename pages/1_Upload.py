...
def _delete_selected_statement(statement_id: int, file_name: str) -> None:
    ...
    if deleted:
        st.session_state["upload_messages"] = [("success", f"Deleted uploaded statement: {file_name}")]
    else:
        st.session_state["upload_messages"] = [("warning", f"Statement already removed: {file_name}")]
    st.rerun()


def _delete_all_statements() -> None:
    deleted_count = delete_all_statements()
    st.session_state["upload_messages"] = [("success", f"Deleted {deleted_count} uploaded statements.")]
    st.rerun()


total_statements = len(statements)
total_transactions = int(statement_transactions["id"].count()) if not statement_transactions.empty else 0
accounts_covered = int(statements["account"].nunique()) if not statements.empty else 0
needs_review = int((statement_transactions["category"] == "Needs Review").sum()) if not statement_transactions.empty else 0
...
st.subheader("Upload")
render_section_lead("Drag and drop one or more PDF bank statements. Newly uploaded files are parsed and reflected immediately.")

with st.container():
    st.caption("Amex and Chase Credit uploads automatically skip page 1 and the last 3 pages before extraction.")
...
if submitted:
    if not files:
        ...
        for file in files:
            ...
            if inserted_count:
                messages.append(("success", f"{file.name}: {result.message} Inserted {inserted_count} transactions."))
            else:
                messages.append(("info", f"{file.name}: {result.message}"))
            if parsed_count == 0:
                messages.append(("warning", f"{file.name}: No transactions were detected. Review the parsing debug panel below."))
